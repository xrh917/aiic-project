...
function App() {
  ...
  const currentQuestion = profile?.agenda?.[qaIndex]?.questions?.[0] || '你希望在研究生阶段继续探索什么问题？';
  const submitAnswer = (e) => { e.preventDefault(); if (!qaAnswer.trim()) return; const nextLog = [...qaLog, { question: currentQuestion, answer: qaAnswer.trim() }]; setQaLog(nextLog); setQaAnswer(''); if (qaIndex + 1 >= Math.min(3, profile.agenda.length)) { const score = Math.max(35, 82 - interruptions.length * 7 - Math.max(0, 60 - seconds)); const next = { recovery: score, interruptionCount: interruptions.length, qaLog: nextLog, duration: Number(form.duration) * 60 - seconds }; setReport(next); sessionStorage.setItem('aiic-report', JSON.stringify(next)); setStage('report'); } else setQaIndex((i) => i + 1); };
  const reset = () => { setStage('setup'); setProfile(null); setReport(null); setForm(initial); sessionStorage.clear(); };
  const clock = `${String(Math.floor(seconds / 60)).padStart(2, '0')}:${String(seconds % 60).padStart(2, '0')}`;
  if (stage === 'presenting' || stage === 'interrupted') return <Presentation clock={clock} profile={profile} segments={segments} interruption={interruption} transcript={transcript} setTranscript={setTranscript} submitSegment={submitSegment} qaAnswer={qaAnswer} setQaAnswer={setQaAnswer} answerInterruption={answerInterruption} endPresentation={endPresentation} />;
