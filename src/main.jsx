...
function App() {
  ...
  const fillDemo = () =>
    setForm({
      ...initial,
      candidateMaterials:
        "我现在是北京某大学信息科学技术学院集成电路设计专业大三，绩点排名大概前 40%。我主要做过三个方向的项目，但这次想重点讲 CircuitGraph。它最开始来自一次 EDA 类比赛，我们要让多模态模型识别非标准的系统级电路图，输出器件连接关系和电路知识问答。比赛之后我们没有停在把分数做上去，而是继续整理数据、重做 workflow，并把它深化成现在这项工作。我在里面主要负责 LoRA 训练和工程落地，包括训练配置、数据版本、实验排期和结果复现，项目管理也主要是我在推进。",
      presentation:
        "各位老师好，我是陈同学，现在是北京某大学信息科学技术学院集成电路设计专业大三，绩点排名大概在前 40%。我平时比较喜欢把一个想法真正跑起来，所以除了模型本身，也会比较关注数据、训练流程和实验能不能稳定复现。今天我想介绍的项目叫 CircuitGraph。这个项目最开始来自一次 EDA 类比赛，题目是让多模态模型理解非标准的系统级电路图。比赛结束以后，我们继续把这个问题做深：一方面整理了 DiagramNet 数据集，另一方面把识别过程拆成感知、推理和知识三个 agent。我的主要工作是用 LoRA 做任务适配和训练落地，包括数据版本管理、训练配置、实验排期和结果复现。具体 workflow 是先用 YOLO 把器件框出来并编号，再让视觉语言模型预测连接关系，最后用任务特定的 LoRA 做知识适配。这样做以后，在我们的基准上总得分是 0.671，迁移到新的模拟电路数据时只用 60 张图像就能重新适配。这个项目让我比较有感觉的一点是，工程流程和模型方法其实是绑在一起的，训练配置、数据质量和评估方式任何一个环节没管好，最后的结果都会不稳定。除了这个项目，我还做过 RTL skill 组合和科研数据集推荐，但今天先不展开。谢谢老师。",
      professorName: "王老师（脱敏画像）",
      affiliation: "北京某高校 · 计算机学院 · 软件研究所",
      researchDirection:
        "群智感知与计算、移动/城市感知、时空数据挖掘、隐私保护、机器学习",
      homepage:
        "长期研究人工智能与普适计算交叉问题，关注群体感知数据的采集、建模、共享和隐私保护，重视数据偏差、实验可复现性与真实场景部署。",
      papers:
        "群智感知任务分配与参与者选择；跨城市时空预测与迁移学习；隐私保护的网络表示学习；联邦学习评估与安全部署",
      duration: 5,
      interruptionMode: "kind",
      demoMode: true,
    });
